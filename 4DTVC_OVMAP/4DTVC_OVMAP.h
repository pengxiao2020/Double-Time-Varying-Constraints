...
//1.DTVC_OVMAP 算法框架
void DTVC_OVMAP(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& Car);		// DTVC_OVMAP 算法框架

//2.DTVC_OVMAP 分配函数
void DTVC_OVMAP_ALLOC(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& UACar);	//DTVC_OVMAP 分配函数
bool EcsInDegree_Ascending(const EcsInfo& ecs1, const EcsInfo& ecs2);	//按基站入度升序对基站排序
void CalcCarBidDensity(EcsInfo& ecs);	//计算：在当前时间槽 t ，能够连接 ecs 的所有车辆的资源密度 Car[i].f
bool CarBidDensity_Descending(const CarInfo& car1, const CarInfo& car2);	//按 车辆投标密度Car[i].f 非递增顺序对车辆排序
bool EscId_Ascending(const EcsInfo& ecs1, const EcsInfo& ecs2);	//根据 ECS_id 升序，重新排列 vEcsInfo 中 ECS 的顺序

//3.DTVC_OVMAP 支付函数（二分法）
