...
void DTVC_OVMAP(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& Car)
{
	//清空容器
	...
	else		//执行 DTVC_OTRAP 算法
	{
		...
		DTVC_OVMAP_ALLOC(cts, ECS, UnAllocatedCar);
		...
		if (!cts_AllocResult.empty())
		{
			...
			DTVC_OVMAP_PAY(cts, vEcsInfo_cp, UnAllocatedCar_cp, cts_AllocResult);
...
void DTVC_OVMAP_ALLOC(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& UACar)
{
	...
	for (auto ecsit = ECS.begin(); ecsit != ECS.end(); ecsit++)
	{
		if ((*ecsit).InDegree != 0)	//仅考察入度不为 0 的基站的情况
		{
			...
			{
				CalcCarBidDensity((*ecsit));	//计算车辆(任务)的价值密度

				//按价值密度非递增顺序，对容器 AbleConnectToECS 中的车辆任务排序
				sort((*ecsit).AbleConnectToECS.begin(), (*ecsit).AbleConnectToECS.end(), CarBidDensity_Descending);

				//根据贪心法，逐个满足车辆的任务资源需求，直到资源不足
				for (auto carit = (*ecsit).AbleConnectToECS.begin(); carit != (*ecsit).AbleConnectToECS.end();)
				{
					////第一种计算形式：DTVC_OTRAP使用（测试成功）
					//if ((*carit).dr > 1)	//资源不够分
					//{
...
void DTVC_OVMAP_PAY(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& UACar, vector<CarInfo>& cts_AR)
{
	...
	for (auto ARCarit = cts_AR.begin(); ARCarit != cts_AR.end(); ARCarit++)	// cts_AR = current time slot_Allocate Result
	{
		...
		while (double(fabs(UB_i - LB_i)) >= epsilon)	//还未求得最小的获胜投标(付款)
		{
			...
			DTVC_OVMAP_ALLOC(cts, Ecs_cp, UACar_cp);
...
void CalcCarBidDensity(EcsInfo& ecs)
{
	//1.计算车辆 i 的主导资源比例
	...
	for (auto carit = ecs.AbleConnectToECS.begin(); carit != ecs.AbleConnectToECS.end();)
	{
		...
		if ((*carit).at <= (*carit).dt - (*carit).et + 1)
		{
			...
			//2.计算分母
			ExecutionTime = (double)(*carit).et;		//记录执行时间et

			request_CPU = (double)(*carit).TRD.cpu;
			request_GPU = (double)(*carit).TRD.gpu;
			request_Memory = (double)(*carit).TRD.memory;
			request_Disk = (double)(*carit).TRD.disk;
...
bool CarBidDensity_Descending(const CarInfo& car1, const CarInfo& car2)
{
...
void SaveResult()
{
	...
	// 保存算法 OTRAP 的求解结果
	string OTRAP_SolutionFile = "../data/4DTVC_OVMAP_Solution_CarInfo_" + to_string(RS_S_CarNum) + "_" + to_string(RS_L_CarNum) + "-" + to_string(ExeNum) + ".txt";
	...
	//测试：找出运行时间长的原因
	ofs << "(1)DTVC_OVMAP 准备工作运行时间：PreWork_RunTime = " << PreWork_RunTime << " ms" << endl;
	...
	ofs << "(2)DTVC_OVMAP 分配函数运行时间：ALLOC_RunTime = " << ALLOC_RunTime << " ms" << endl;
	ofs << "(3)DTVC_OVMAP 支付函数运行时间：PAY_RunTime = " << PAY_RunTime << " ms" << endl;
	ofs << endl;
...
int main()
{
	...
	//for (; ExeNum <= count_DTVC_OVMAP; ExeNum++)
	if (ExeNum <= count_DTVC_OVMAP)		//检查 bug 使用
	{
		cout << "这是第 " << ExeNum << " 次运行 DTVC_OVMAP 算法 ! " << endl;
		...
		cout << "在时间槽[1, " << T << "]，开始运行 DTVC_OVMAP 算法！" << endl;

		time_t tBegin, tEnd;
		tBegin = clock();	//开始计时

		//第二步：每个时间槽执行一次 DTVC_OVMAP 算法
		for (int t = 1; t <= T; t++)
		{
			//cout << "当前时间槽 t = " << t << ", 执行 DTVC_OTRAP 算法：" << endl;
			DTVC_OVMAP(t, vEcsInfo, T_vCarInfo);
		}

		tEnd = clock();	//计时结束

		cout << "第 " << ExeNum << " 次运行 DTVC_OVMAP 算法结束！" << endl;
		cout << endl;
