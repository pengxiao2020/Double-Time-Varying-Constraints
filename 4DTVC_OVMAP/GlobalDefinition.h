...
//(2)时间槽设置
#define TTS 1440	//TTS = Total Time Slot
const int T = TTS;
...
// 3.车辆信息相关
#define RS_S_CarNum 5000		//读入的已经随机抽取的小型车辆任务数		
#define RS_L_CarNum 5000		//读入的已经随机抽取的大型车辆任务数
...
// 4.读入文件定义
const int count_DTVC_OVMAP = 10;	//DTVC_OVMAP 机制 总的执行次数, 设为 10
int ExeNum = 1;						//第几次运行 DTVC_OVMAP 机制，初始值为 1

//基站的 基本信息 和 资源容量矩阵 分开保存
#define EcsInfoFile "../data/2RandomSampling/data_EcsInfo_" + to_string(S_EcsNum) + "_" + to_string(L_EcsNum)+ ".txt"
#define EcsInfoFile_RCM "../data/2RandomSampling/data_Ecs_RCM_" + to_string(S_EcsNum) + "_" + to_string(L_EcsNum)+ ".txt"

//车辆的 任务信息, 时变资源请求 和 时变部署约束矩阵 分开保存
#define RS_CarInfoFile_S "../data/2RandomSampling/data_RS_CarInfo_S_"+ to_string(RS_S_CarNum) + "-" + to_string(ExeNum) + ".txt"		//小型车辆任务信息
#define RS_CarInfoFile_S_RDM "../data/2RandomSampling/data_RS_Car_RDM_S_"+ to_string(RS_S_CarNum) + "-" + to_string(ExeNum) + ".txt"	//小型车辆的资源需求矩阵
#define RS_CarInfoFile_S_DCM "../data/2RandomSampling/data_RS_Car_DCM_S_"+ to_string(RS_S_CarNum) + "-" + to_string(ExeNum) + ".txt"	//小型车辆的部署约束矩阵

#define RS_CarInfoFile_L "../data/2RandomSampling/data_RS_CarInfo_L_"+ to_string(RS_L_CarNum) + "-" + to_string(ExeNum) + ".txt"		//大型车辆任务信息
#define RS_CarInfoFile_L_RDM "../data/2RandomSampling/data_RS_Car_RDM_L_"+ to_string(RS_L_CarNum) + "-" + to_string(ExeNum) + ".txt"	//大型车辆的资源需求矩阵
#define RS_CarInfoFile_L_DCM "../data/2RandomSampling/data_RS_Car_DCM_L_"+ to_string(RS_L_CarNum) + "-" + to_string(ExeNum) + ".txt"	//大型车辆的部署约束矩阵

// 5.程序使用到的变量和定义
const double epsilon = 0.000001;	//定义一个较小的数字，DTVC_OVMAP_PAY 确定付款使用
